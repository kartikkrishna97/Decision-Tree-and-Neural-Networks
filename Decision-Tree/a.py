...
X, y, type, attributes = preprocess_data("d-tree/train.csv")
X_val, Y_val, type, attributes = preprocess_data('d-tree/validation.csv')
X_test, Y_test, type, attributes = preprocess_data('d-tree/test.csv')
...
class DtreeNode:
    def __init__(self, dataPoints, labels):
        ...
        self.cutoff = None
        self.result = None
        self.isDiscrete = None


    def growTree(self, TreeDepth, DepthLimit, type):
        ...
        self.result = 1 if positive_labels > negative_labels else 0
        bestFeat, cutoff = best_attribute(self.dataPoints, self.labels, type)
        if bestFeat == -1:
            self.leafNode = True
            return
        if cutoff is None:
            ...
            if bestFeat in type:
                ...
                uniqueValues = np.unique(self.dataPoints[:,bestFeat])
                for value in uniqueValues:
                        dpSubset = self.dataPoints[self.dataPoints[:, bestFeat] == value]
                        labelSubset = self.labels[self.dataPoints[:, bestFeat] == value]
                        self.children[value] = DtreeNode(dpSubset, labelSubset)
                        self.children[value].growTree(TreeDepth + 1, DepthLimit, type)
            ...
            self.children[0].growTree(TreeDepth + 1, DepthLimit, type)
            ...
            self.children[1].growTree(TreeDepth + 1, DepthLimit, type)
...
def check_accuracy(dataset, actualLabels, root):
    ...
    for data in dataset:
        ...
        while not current_node.leafNode:
            feature_idx = current_node.bestFeat
            feature_value = data[feature_idx]
            
            if current_node.isDiscrete: 
                ...
                if feature_value <= current_node.cutoff:
    ...
    successCount = sum(1 for i in range(len(predictedLabels)) 
                      if predictedLabels[i] == actualLabels[i])
    totalAccuracy = successCount / len(actualLabels) * 100

    zeroClassAccuracy = sum(1 for correct, estimated in zip(actualLabels, predictedLabels) 
                           if correct == 0 and estimated == 0)
    zeroClassTotal = np.sum(actualLabels == 0)
    
    oneClassAccuracy = sum(1 for correct, estimated in zip(actualLabels, predictedLabels) 
                          if correct == 1 and estimated == 1)
